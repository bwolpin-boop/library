...
## Step 1 — Identify nested components

Before building anything, look at the component design (from Figma or from the name/description) and identify any sub-components it contains that are meaningful standalone pieces — e.g. a Label, Badge, Avatar, Tag, Icon, or any element that looks like it belongs to its own component family.

For each nested piece found:
1. Check whether it already exists in `src/components/`
2. If it **does not exist**, ask the user before continuing:

   > **"[ComponentName] contains a [NestedName] element that doesn't exist yet as its own component. Should I create the full [NestedName] component set first (component + stories + overview + export), then use it inside [ComponentName]?"**

If the user says yes, run this skill recursively for the nested component first, then return to building the parent.
If the user says no, build the nested piece inline as a local element inside the parent component.

Do not assume — always ask, even if the nested piece seems small.

---

## Step 2 — Identify the Figma page (for Storybook naming)
...
## Step 4 — NavIcon audit (if the component uses icons)
...
## Step 5 — Create the component file
...
## Step 6 — Create the variants story file
...
## Step 7 — Create the Overview story file
...
## Step 8 — Add export to src/index.js
...
## Step 9 — Update AllComponents.stories.jsx
