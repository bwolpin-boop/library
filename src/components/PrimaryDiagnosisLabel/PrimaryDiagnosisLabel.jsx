...
function HexagonIcon({ color, size = 10 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" className="dc:shrink-0">
      <path
...
const VARIANTS = {
  ...
  'primary-set': {
    bg:         'rgba(168,82,255,0.1)',
    border:     'none',
  },
  'primary-unset': {
    bg:         '#ffffff',
    border:     '0.658px solid #e8e8ec',
  },
  'primary-active': {
    bg:         '#a852ff',
    border:     'none',
...
export function PrimaryDiagnosisLabel({
  ...
  return (
    <div
      className="dc:inline-flex dc:items-center dc:justify-center dc:box-border"
      style={{
        gap:            3,
        height:         20,
        padding:        '0 5px',
    ...
    >
      ...
      {type === 'nta-blue' && hasText && (
        <span className="dc:font-montserrat dc:text-xxxs dc:font-semibold dc:whitespace-nowrap" style={{ lineHeight: 'normal', color: '#65B7C5' }}>NTA</span>
      )}

      {type === 'nta-yellow' && (
        <>
          {hasIcon && <DiamondIcon color="#F1B900" size={8} />}
          {hasText  && <span className="dc:font-montserrat dc:text-xxxs dc:font-semibold dc:whitespace-nowrap" style={{ lineHeight: 'normal', color: '#F1B900' }}>NTA</span>}
        </>
      )}

      {type === 'primary-set' && (
        <>
          {hasIcon && <HexagonIcon color="#a852ff" size={10} />}
          {hasText  && <span className="dc:font-montserrat dc:text-xxxs dc:font-semibold dc:whitespace-nowrap dc:text-purple" style={{ lineHeight: 'normal' }}>Primary</span>}
        </>
      )}

      {type === 'primary-unset' && (
        <>
          {hasIcon && <HexagonIcon color="#8c8ca1" size={10} />}
          {hasText  && <span className="dc:font-montserrat dc:text-xxxs dc:font-semibold dc:whitespace-nowrap dc:text-secondary" style={{ lineHeight: 'normal' }}>Primary</span>}
        </>
      )}

      {type === 'primary-active' && (
        <>
          {hasIcon && <HexagonIcon color="#ffffff" size={10} />}
          {hasText  && <span className="dc:font-montserrat dc:text-xxxs dc:font-semibold dc:whitespace-nowrap dc:text-white" style={{ lineHeight: 'normal' }}>Set as Primary Diagnosis</span>}
        </>
      )}
    </div>
  )
